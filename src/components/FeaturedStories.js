...
import FeaturedStoriesCard from './FeaturedStoriesCard';
import FeaturedStoriesData from './FeaturedStoriesData';
function FeaturedStories() {
  return (
    <div>
      ...
      <div className='justify-between grid-cols-3 gap-4 flex flex-col md:flex-row'>
        {FeaturedStoriesData.map(card => (
          <FeaturedStoriesCard card={card} key={card.link}/>
        ))}
      </div>
    </div>
  );
